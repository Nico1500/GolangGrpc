...
func pbEntryToDatabaseEntry(pbEntry *pb.EmailEntry) database.EmailEntry {
	t := time.Unix(pbEntry.ConfirmedAt, 0)
	return database.EmailEntry{
		Id:          pbEntry.Id,
		Email:       pbEntry.Email,
		ConfirmedAt: &t,
...
func databaseEntryToPbEntry(mdbEntry *database.EmailEntry) pb.EmailEntry {
...
func emailResponse(db *sql.DB, email string) (*pb.EmailResponse, error) {
	entry, err := database.GetEmail(db, email)
	if err != nil {
	...
	res := databaseEntryToPbEntry(entry)
...
func (s *MailServer) GetEmailBatch(ctx context.Context, req *pb.GetEmailBatchRequest) (*pb.GetEmailBatchResponse, error) {
	...
	params := database.GetEmailBatchQueryParams{
		Page:  int(req.Page),
		Count: int(req.Count),
	}

	mdbEntries, err := database.GetEmailBatch(s.db, params)
	if err != nil {
	...
	for i := 0; i < len(mdbEntries); i++ {
		entry := databaseEntryToPbEntry(&mdbEntries[i])
...
func (s *MailServer) CreateEmail(ctx context.Context, req *pb.CreateEmailRequest) (*pb.EmailResponse, error) {
	...
	err := database.CreateEmail(s.db, req.EmailAddr)
	if err != nil {
		return &pb.EmailResponse{}, err
	}
...
func (s *MailServer) UpdateEmail(ctx context.Context, req *pb.UpdateEmailRequest) (*pb.EmailResponse, error) {
	...
	entry := pbEntryToDatabaseEntry(req.EmailEntry)

	err := database.UpdateEmail(s.db, entry)
	if err != nil {
		return &pb.EmailResponse{}, err
	}
...
func (s *MailServer) DeleteEmail(ctx context.Context, req *pb.DeleteEmailRequest) (*pb.EmailResponse, error) {
	...
	err := database.DeleteEmail(s.db, req.EmailAddr)
	if err != nil {
		return &pb.EmailResponse{}, err
	}
